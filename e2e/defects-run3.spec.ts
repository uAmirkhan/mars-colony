...
test('Д-18: докупка после частичной погрузки запирает товар на складе', async ({ page }) => {
  ...
  // Трата изотопов необратима, и каркас (раздел 13) требует подтверждения.
  // Без этого шага докупка не происходит вовсе, рейс не уходит, и проверка
  // краснеет ровно тем же складом, что и на исходном дефекте — то есть врет
  // про его возвращение.
