...
describe('Кредитные стоки: деньги обязаны тратиться', () => {
  ...
  it('лёгкий профиль за месяц берёт три здания из четырёх', () => {
    // История этого теста в трёх ходах.
    //
    // Круг 2: при 4 часах в неделю третье здание за месяц недостижимо, рецепты
    // ткани и комбинезона не открываются вообще.
    //
    // Добыча это починила побочно — её добавляли ради согласия мира с
    // экономикой, а не ради баланса. Буровая даёт доход, не требуя ни кредитов
    // на посев, ни слота грядки, то есть работает и когда игрок не заходит.
    // Тест тогда перевернули: он сторожил, чтобы ослабление добычи не вернуло
    // дыру молча.
    //
    // Смена лестницы времени роста (решение Khan'а 08.09) вернула дыру, и
    // вернула её НЕ через добычу: времена добычи не менялись. Капуста зреет
    // в 7.5 раза дольше при той же цене, её доход упал с 210 до 28 кредитов
    // в час грядки, и лёгкий профиль перестал успевать.
    //
    // Проверено и не помогает: цена капусты (3,4,5,6), её урожай (8,12,16),
    // цена самого модуля (5000, 4500, 4000 — без изменений вообще), передача
    // первого уровня сое (стало хуже). Цена модуля не двигает результат, и это
    // главное: упирается не в деньги, а во время — девятый уровень приходит
    // слишком поздно, месяц кончается раньше покупки.
    //
    // Решение: принято как есть. Месяц не предел жизни игрока.
    //
    // Сторожевая функция сохранена: состояние записано числами. Ослабят добычу
    // или тронут экономику — числа разъедутся, и тест покраснеет.
    const r = simulate(base);
    expect(r.final_level).toBe(9);
    expect(r.buildings_owned).toEqual(
      expect.arrayContaining(['food_module', 'mining_site', 'atmospheric_module']),
    );
    expect(r.buildings_owned).not.toContain('textile_module');
    expect(r.rows.at(-1)?.credits ?? 0).toBe(5005);
  });
...
describe('Достижимость товаров: по потолку и на практике — разные вещи', () => {
  it('целевой профиль за 30 дней не открывает часть товарного субстрата', () => {
    // Тест записывает ФАКТ, а не требование: месяц не предел жизни игрока.
    // Он существует, потому что «товар есть в конфиге» читается как «товар в
    // игре есть», а это не одно и то же.
    //
    // Факт до 08.09: целевой профиль доходил до одиннадцатого уровня, то есть
    // кофе-бобы (12) и кофе-паёк (13) игрок не видел вообще, а комбинезон (11)
    // открывался в последний день.
    //
    // Факт после смены лестницы времени роста (решение Khan'а 08.09): профиль
    // доходит до тринадцатого. Причина — томаты: они зрели час, стали зреть
    // двадцать минут, а дают шесть очков опыта против одного у сои. Ускорение
    // самой доходной по опыту культуры втрое подняло прогрессию на два уровня.
    //
    // Теперь весь товарный субстрат открывается за месяц. Это НЕ улучшение по
    // умолчанию: игрок, увидевший всё содержимое за тридцать дней, дальше
    // держится только повторением. Вопрос удержания вынесен владельцу, тест
    // лишь не даёт факту исчезнуть молча.
    const target = {
      ...base,
      session_starts_min: [480, 760, 1040, 1320],
      session_length_min: 20,
