...
/**
 * Всплывающий текст «+N»: [[ux-motion-spec]] раздел 2, строка «Всплывающий
 * текст «+N XP/кредитов» — 300 мс подъем + fade».
 *
 * Здесь стояло 900 при том же комментарии «не выдуманные» — то есть код
 * утверждал соответствие спеке втрое дольше самой спеки (находка Н-14). Такое
 * хуже простого расхождения: комментарий гасит подозрение у следующего
 * читателя, и место перестает проверяться вовсе.
 *
 * Спека не говорит, входит ли fade в эти 300 мс или идет сверх них. Читаем
 * как «весь эффект укладывается в 300»: цифра всплывает НА месте действия, где
 * игрок уже смотрит, и долгий хвост там читается мусором, а не наградой.
 * Решение записано в реестр [[spec-prototype-build]], раздел 8.
 */
export const FLOAT_MS = 300;
