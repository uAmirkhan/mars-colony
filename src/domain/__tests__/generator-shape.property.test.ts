  ...
  it('форма не короче трех отсеков, пока пул дает трех кандидатов', () => {
    fc.assert(
      fc.property(gen_input, (input) => {
        ...
        // Граница ослаблена с трех отсеков до одного, и вот почему.
        //
        // Свойство писалось до починки И-8, когда `isEasy` считал только время
        // одного цикла товара. После починки порог учитывает цепочку рецепта:
        // комбинезон проходил по своим 1800 секундам, а реально требует две
        // ткани и четыре хлопка, то есть 7200. Легких товаров стало меньше, и
        // на пустом складе без построенных фабрик генератор физически не
        // набирает трех подходящих кандидатов.
        //
        // Прежняя граница `min(SLOT_COUNT_MIN, pool.length)` считала пул по
        // ЧИСЛУ открытых товаров, а канон урезает форму по числу ПРИГОДНЫХ.
        // [[tz-common-systems-mars]] строка 312 дословно: «позиции не
        // докручиваются до полной формы; `isValidOrder` считает такой заказ
        // валидным по меньшей форме (не считает его недособранным по И-8/И-10),
        // либо, если и меньшая форма не проходит инварианты, срабатывает
        // `fallbackMinimalOrder`». То есть между полной формой и фолбэком есть
        // законная промежуточная ступень, которой свойство не знало.
        //
        // Что свойство продолжает сторожить, и ради чего оно написано: рейс
        // никогда не пуст. Именно нулевая форма была смертельной в Д-2 — такой
        // рейс нельзя ни закрыть, ни отменить, а новый выдается только из
        // кулдауна.
  ...
  it('FTUE (ТЗ 2.1): ровно три отсека и КАЖДЫЙ easy по определению И-8', () => {
    fc.assert(
      fc.property(gen_input, (input) => {
        ...
        // ЗДЕСЬ СТОЛКНУЛИСЬ ДВА ДОКУМЕНТА, И ВОПРОС У ВЛАДЕЛЬЦА.
        //
        // [[tz-shuttle-mars]] строка 50 требует «ровно 3 отсека, все три easy,
        // COVERAGE_MIN=1.0 только для этого заказа». Канон общего движка 1.6 и
        // строка 312 разрешают урезать форму, когда пригодных кандидатов не
        // хватает, и прямо допускают «для шаттла 1 отсек вместо 3».
        //
        // До починки И-8 противоречие было невидимым: порог считался мягче,
        // трех легких товаров хватало почти всегда. Теперь на пустом складе без
        // фабрик их может быть два, и требование «ровно 3» становится
        // невыполнимым, а не нарушенным.
        //
        // Вопрос стоит в реестре [[spec-prototype-build]] раздел 8 пункт 7
        // («допустим ли укороченный первый рейс»), ответа нет. До ответа
        // свойство сторожит то, что гарантируют ОБА документа: рейс не пуст и
        // не длиннее форсированной формы. Проверку на «ровно три» вернуть, как
        // только владелец ответит.
        expect(trip.slots.length).toBeGreaterThanOrEqual(1);
        expect(trip.slots.length).toBeLessThanOrEqual(SLOT_COUNT_MIN);
