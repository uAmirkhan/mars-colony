...
describe('Дефект: rebalanceForAchievability (И-10) не знает о deficit_locks (И-13)', () => {
  it('замена товара не должна попадать на товар, уже залоченный ДРУГОЙ механикой', () => {
    ...
    // кандидатами до guard_limit — отдельный дефект, см. следующий describe).
...
describe('Дефект: rebalanceForAchievability не сходится к лучшему увиденному варианту', () => {
  /**
   * Найдено по прямому запросу координатора после приемки дефектов 1-3.
   *
   * Когда НИ ОДИН кандидат замены не укладывается в бюджет сам по себе, цикл
   * не запоминает лучшее (минимальное по времени) состояние, увиденное за
   * перебор, — он просто возвращает то, что осталось на слоте В МОМЕНТ
   * остановки `guard_limit` (`GEN_MAX_ATTEMPTS x SLOT_COUNT_MAX` = 200).
   * Поскольку `used` отслеживает только ТЕКУЩИЙ состав слотов, а не историю
   * уже отвергнутых замен, товар, от которого только что ушли (потому что он
   * не влезал в бюджет), становится СНОВА доступным кандидатом на следующей
   * итерации — цикл может ходить по кругу между несколькими кандидатами, и
   * итоговый результат определяется четностью числа итераций до
   * `guard_limit`, а не тем, какой из увиденных вариантов объективно ближе к
   * бюджету.
   *
   * Демонстрация на трех кандидатах: `mushroom_soup` (50 мин на полу) строго
   * лучше `coffee_ration` (82.5 мин) и `jumpsuit` (120 мин), функция его
   * РЕАЛЬНО посещает во время перебора (см. debug-прогон при расследовании) —
   * и все равно останавливается на `coffee_ration`, худшем из трех
   * посещенных вариантов, просто потому что четность `guard_limit` угадала
   * на него.
   *
   * Проверено на альтернативной реализации той же функции с добавленным
   * "запоминанием лучшего состояния" (см. рабочие заметки тестировщика) —
   * такая реализация на этом же сценарии сходится к `mushroom_soup`
   * корректно, то есть свойство достижимо и не является структурным
   * ограничением алгоритма, а именно упущением текущей реализации.
   */
  it('на нескольких недостижимых кандидатах может вернуть худший из посещенных, а не лучший', () => {
    const w = createWarehouse(500);
    const slots = [slot('jumpsuit', 1)];
    // Бюджет заведомо недостижим для всех трех кандидатов — структурный
    // случай реестра 8.24, но результат обязан быть НЕ ХУЖЕ, чем лучший
    // реально посещенный вариант.
    const result = rebalanceForAchievability(slots, w, 5, [
      'jumpsuit',
      'mushroom_soup',
      'coffee_ration',
    ]);

    const best_visited_minutes = Math.min(
      productionTimeMinutes('jumpsuit', 1, w),
      productionTimeMinutes('mushroom_soup', 2, w), // GOOD_BASE_QTY.mushroom_soup.min
      productionTimeMinutes('coffee_ration', 1, w), // GOOD_BASE_QTY.coffee_ration.min
    );

    expect(tripProductionMinutes(result, w)).toBeLessThanOrEqual(best_visited_minutes);
  });
});
