...
/**
 * Потребность в модулях для дроп-роллера: сколько модулей просят рецепты строек,
 * которые игрок может начать прямо сейчас.
 *
 ...
 * (6 и 7), канон дает 13, максимум возвращал 7.
 *
 * **Валовая, а не остаток после склада.** Склад из суммы НЕ вычитается, и это
 * не небрежность: обе формулы И-7/И-11 сравнивают с этим числом сам запас.
 * [[tz-common-systems-mars]] 2.3: `activeNeed = activeContextNeed(item,
 * colony_state)` и следом `if activeNeed > 0 and avgStock24h > activeNeed *
 * config.ANTISTOCKPILE_THRESHOLD`; 2.6 п.3: «`warehouseCoversConstruction` —
 * сравнение `ModuleStock.qty` игрока с оставшейся потребностью рецепта
 * стройки». У стройки в AVAILABLE ничего не внесено (3.4 п.2: «`ModuleStock` не
 * имеет `reserved`»), поэтому «оставшаяся потребность рецепта» — весь рецепт.
 * Если вычесть склад здесь, обе формулы сравнивают запас с самим собой: порог
 * анти-стокпайла вырождается в «запас больше своей же нехватки» и бьет по
 * модулю, которого не хватает (Д-25), а покрытие И-11 вырождается в «запас
 * больше половины рецепта» и гасит гарантию на почти собранном комплекте
 * (Д-24). Обратной дороги у вычитания нет: дефицит выводится из пары
 * (потребность, склад), а рецепт из дефицита — уже нет.
 *
 * Дефицит по конкретной стройке — `missingFor`; чек-листы интерфейса берут его,
 * а не эту функцию.
 */
