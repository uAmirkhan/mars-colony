...
/**
 * Канон 1.8: причины деградации генератора заказов. Перечисление живет в
 * конфиге, а не литералом в точке вызова: по нему считается метрика «частота
 * фолбэков», и она различает баг генератора (`max_attempts`,
 * `unresolvable_invariant`) и недостроенный контент-пул (`empty_pool`).
 * Зашитый литерал сливает два разных сигнала в один.
 */
export const ORDER_GENERATION_DEGRADED_REASONS = [
  'empty_pool',
  'max_attempts',
...
/**
 * Канон 1.7: `ACHIEVABILITY_CHECK[mechanic]` — включена ли проверка И-10 для
 * механики. Выключена у дрона (нет дедлайна: заказы висят на доске без
 * таймера, каркас раздел 7). Лайнер в срезе не реализован (реестр
 * [[spec-prototype-build]] раздел 8), но имя читается по механике целиком,
 * не только по реализованным — так его находит `spec-drift.test.ts`.
 */
export const ACHIEVABILITY_CHECK: Record<Mechanic, boolean> = {
  drone: false,
  shuttle: true,
  liner: true,
};
...
/**
 * Канон 1.5/1.7: потолок TTL лока И-13 — даже забытый дрон-заказ («заказы
 * висят», каркас раздел 7) отпускает лок через сутки, чтобы не блокировать
 * шаттл/лайнер навсегда.
 */
export const DEFICIT_LOCK_TTL_MAX = 24 * 60 * 60;

/**
 * Канон 1.5: `ttl` — время жизни лока «до терминального события заказа».
 * Штатно лок снимается раньше явным `releaseDeficitLock` (deliver/discard/
 * depart — терминальный переход заказа-владельца); значение по умолчанию
 * равно потолку, потому что нормальный путь снятия не зависит от TTL вовсе —
 * TTL здесь чистый предохранитель на случай, если терминальное событие не
 * случилось (брошенный дрон-заказ).
 */
export const DEFICIT_LOCK_TTL = DEFICIT_LOCK_TTL_MAX;

// --- Дрон: доска, выброс и рефреш ----------------------------------------

/**
 * ТЗ дрона 4.1: сколько карточек заказов видно на доске, по уровню колонии.
 * Верхняя граница доски, а не цель наполнения: генератор наполняет каждый
 * освободившийся слот отдельным вызовом. Потолок 9 фиксирован каркасом 7.
 */
export const NUM_VISIBLE_ORDERS: Array<{ from_level: number; orders: number }> = [
  { from_level: 15, orders: 9 },
  { from_level: 12, orders: 8 },
  { from_level: 10, orders: 7 },
  { from_level: 8, orders: 6 },
  { from_level: 6, orders: 5 },
  { from_level: 4, orders: 4 },
  { from_level: 2, orders: 3 },
];
...
/**
 * Премия дрона к рыночной цене: клемп итогового множителя.
 *
 * **Источники расходились, закрыто владельцем 2026-08-05: правит каркас.**
 * Каркас (раздел 7, «Платит») пишет «кредиты с премией +25-70% к рынку
 * (медиана ~45%)», то есть клемп [1.25, 1.70]. ТЗ дрона (4.3 и конфиг-таблица
 * 6) пишет `clamp(premium, 1.35, 1.65)` и ссылается на тот же раздел 7 каркаса
 * как на источник («диапазон фиксирован каркасом — не переопределяется этим
 * ТЗ»). Два документа задают разные числа, каждый со ссылкой на другой.
 *
 * Принято значение каркаса: `mars-colony/CLAUDE.md` объявляет каркас источником
 * чисел («противоречие каркасу — баг кода»), а ТЗ дрона цитирует каркас
 * неточно. Практическая разница узкая: слагаемые ТЗ 4.3 дают максимум 1.68 и
 * минимум 1.36, то есть режется только верхний угол на заказе, где сошлись
 * фабричный товар, короткая форма и дефицитная позиция.
 *
 * Владелец подтвердил выбор каркаса 2026-08-05. Правится документ, а не код:
 * ТЗ дрона приводится к каркасу, иначе расхождение вернется при следующем
 * чтении. Реестр [[spec-prototype-build]] раздел 8, пункт 8 — закрыт.
 */
export const DRONE_PREMIUM_RANGE = { min: 0.25, max: 0.7 };

/**
 * Надбавка к премии за целевую дефицитную позицию ([[tz-drone-mars]] 4.3,
 * строка «Бонус целевого дефицита» конфиг-таблицы раздела 6, диапазон тюнинга
 * +0.03..+0.08).
 *
 * Слагаемое — плата за то, что заказ просит товар, которого у игрока нет:
 * И-8 разрешает одну такую позицию на заказ, и без надбавки дефицитный заказ
 * платит столько же, сколько заказ из одного склада, при этом требуя
 * производственного цикла.
 */
export const DRONE_PREMIUM_DEFICIT_BONUS = 0.05;
