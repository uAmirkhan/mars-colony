...
export const GOODS: Record<GoodId, Good> = {
  ...
  oxygen_tank: {
    ...
    base_xp: 9,
    prod_time_sec: 600,
    // Кислород берут из оксидов железа, а не из капусты (Khan 07.09): связка с едой
    // читалась как ошибка. Цена поднята вслед за подорожавшим входом, маржа прежняя.
    inputs: [{ good_id: 'iron_ore', qty: 1 }],
  ...
  // Появились 2026-09-03: в сцене четыре точки добычи, а ресурса было два, и
  // две точки добывали одно и то же. Дубль ничего не дает игроку, кроме
  // путаницы, поэтому каждая точка получила свой ресурс: карьер — руду,
  // буровая на леднике — метан, скважина — кислород, сборщики — лед.
  //
  // Руда, а не «металл вообще»: марсианский грунт красный из-за оксида железа,
  // и открытый карьер по гематиту — то, из чего планета состоит под ногами.
  // Метан из подледных клатратов: Curiosity фиксировал сезонные выбросы, и
  // это единственное объяснение, при котором вышка стоит на льду не случайно.
  //
  // И-1 не нарушается: ни руда, ни метан не превращаются в строй-модули,
  // шаттл остается гейтом прогрессии. Числа держат И-2: 4/10 = 0.40,
  // 11/25 = 0.44.
  iron_ore: {
    // Уровень 7, а не 6: на шестом руда садится рядом с реголитом и льдом и
    // меняет состав ранних заказов так, что щедрые награды за уровень
    // перестают опережать скупые (sim/tuning). Проверено перебором:
    // уровни 7 и 8 инвариант держат, 6 и 9 ломают.
    id: 'iron_ore',
    name: 'Железная руда',
    kind: 'factory',
    unlock_level: 7,
  ...
  // Первая переработка добытого сырья в игре: до нее лед и реголит были
  // тупиками, их можно было только продать. Наценка 28/14 = 2.0x — середина
  // существующей вилки (1.43x у кофе-пайка, 4.62x у ткани). За час выработки
  // продажа льда дает 84 кредита, переработка в воду — 100: перерабатывать
  // выгоднее, чем продавать сырье, иначе здание не нужно.
  water: {
    id: 'water',
    name: 'Вода',
    kind: 'factory',
    ...
    base_xp: 12,
    prod_time_sec: 400,
    inputs: [{ good_id: 'water_ice', qty: 1 }],
    required_building: 'water_plant',
  },
};
