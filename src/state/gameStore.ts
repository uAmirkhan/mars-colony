 ...
 *
 * **6 — форма `DeficitLock` сменила `order_ref: string` на `owners: string[]`**
 * (прогон 6, дефект: лок держался на паре (товар, механика), а не на паре
 * (товар, заказ-владелец) — терминация ОДНОГО заказа снимала защиту с
 * ЖИВОГО заказа-соседа той же механики, легально державшего тот же
 * дефицитный товар, канон 1.5). Сейв версии 5 несет старую форму
 * (`order_ref`), а код версии 6 читает `owners` — молчаливая подстановка
 * невозможна по тому же правилу, что версии 2-5: старую запись нельзя
 * реконструировать в множество владельцев (список остальных заказов той же
 * механики, деливших тот же товар, в старом сейве не сохранен).
 *
    ...
    (set, get) => {
      ...
      return {
        ...
        refreshSlotNow: (slot_idx) => {
          ...
          orders[slot_idx] = makeOrder(slot_idx, s.now);
          // И-13: makeOrder мутирует s.deficit_locks по месту (см. докстринг
          // makeOrder) — обязана новая ссылка, иначе подписчик на поле не
          // узнает об изменении (тот же класс дефекта, что уже чинили у
          // buyModulesFor/startConstruction на поле purchased).
          set({ orders, isotopes: s.isotopes - price, deficit_locks: { ...s.deficit_locks } });
        },
