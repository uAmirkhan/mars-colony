...
export default defineConfig({
  testDir: './e2e',
  /*
   * Сроки подняты сознательно. Проверки гоняются на машине, где параллельно
   * работают агенты, и при полной нагрузке шесть воркеров упирались в
   * пятисекундный срок ожидания — падало каждый прогон по-разному, а поодиночке
   * все проходило. Мигающая проверка хуже отсутствующей: она приучает не верить
   * красному. Срок — это не утверждение теста, поэтому запас здесь ничего не
   * ослабляет.
   */
  timeout: 45_000,
